...
export class ListasComponent implements OnInit, OnDestroy {
  ...
  private expandedListas: Set<string> = new Set();
  ...
  ngOnInit(): void {
    ...
    if (this.esGestor && localStorage.getItem('gestor_data')) {
      this.actualUser = JSON.parse(localStorage.getItem('gestor_data')!);
    } else if (this.esAdministrador && localStorage.getItem('admin_data')) {
      this.actualUser = JSON.parse(localStorage.getItem('admin_data')!);
