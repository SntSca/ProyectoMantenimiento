...
export class SharedService {
  ...
  getTags(): Observable<Tags[]> {
    //Implementar llamada HTTP GET a la API para obtener tags
    return of (MOCK_TAGS);
  }

  getEspecialidades(): Observable<Especialidades[]> {
    //Implementar llamada HTTP GET a la API para obtener especialidades
  ...
  /**
   * Cierra la sesión del usuario eliminándola del backend y limpiando el localStorage
   */
  ...
  clearSession(): void {
    ...
    localStorage.removeItem('gestor_data');
    
  }

  // Método para convertir File a base64 (solo el cuerpo, sin prefijo)
  ...
  // Método para convertir base64 a File (monta la cabecera internamente si no está presente)
  base64ToFile(base64String: string, mimeType?: string): Promise<File> {
    return new Promise((resolve, reject) => {
      try {
        // Extraer la parte base64 después de la coma, si incluye el prefijo
  ...
  // Método para configurar búsqueda dinámica con debounce
  setupDynamicSearch(searchSubject: Subject<string>, callback: (searchTerm: string) => void, destroy$: Subject<void>): void {
    searchSubject
      .pipe(
        ...
        takeUntil(destroy$) // Limpiar suscripción al destruir componente
      )
  ...
  // Método para manejar logout con confirmación y navegación
  ...
  /**
   * Obtiene las credenciales del usuario desde el token JWT almacenado en localStorage
