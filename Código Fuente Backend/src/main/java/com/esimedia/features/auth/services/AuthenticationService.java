...
public class AuthenticationService {
    ...
    private final Map<String, AuthSession> authSessions = new HashMap<>();
    ...
    public Usuario authenticatePrivilegedUser(Map<String, String> credentials) {
        ...
        if (creador != null) {
            logger.info("[DEBUG] Intentando login para creador: {} (validado: {})", creador.getEmail(), creador.isValidado());
            if (!creador.isValidado()) {
                logger.warn("[DEBUG] Creador no validado");
                throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, "El creador de contenido no ha sido validado por un administrador");
            }
            if (creador.isBloqueado()) {
                throw new ResponseStatusException(HttpStatus.FORBIDDEN, CUENTA_BLOQUEADA_MSG);
            }
            boolean passwordMatch = matchesPassword(password, creador.getPassword());
            logger.info("[DEBUG] Comparando password para {}: {}", creador.getEmail(), passwordMatch);
            if (!passwordMatch) {
                logger.warn("[DEBUG] Password incorrecta para {}", creador.getEmail());
                throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, CREDENCIALES_INVALIDAS);
            }
        ...
        if (admin != null) {
            ...
            logger.info("[DEBUG] Comparando password para admin {}: {}", admin.getEmail(), passwordMatch);
            if (!passwordMatch) {
                logger.warn("[DEBUG] Password incorrecta para admin {}", admin.getEmail());
                throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, CREDENCIALES_INVALIDAS);
            }
    ...
    private static class AuthSession {
        ...
        private static final long EXPIRATION_MS = 15L * 60 * 1000;

        public AuthSession(String userId, int currentStep) {
            this.userId = userId;
