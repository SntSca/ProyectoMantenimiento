...
public class AuthController {
    ...
    public ResponseEntity<Object> privilegedLogin(@RequestBody Map<String, String> credentials, HttpServletRequest request) {
        String ip = request.getRemoteAddr();
        String username = credentials.getOrDefault(EMAIL_FIELD, "").trim();

        logger.info("[LOGIN-PRIVILEGED] Petición recibida en /users/auth/privileged-login para usuario: {} desde IP: {}", username, ip);

        // Validación básica de email
        if (username.isEmpty()) {
            logger.warn("[LOGIN-PRIVILEGED] Email vacío en petición de login privilegiado desde IP: {}", ip);
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body("El campo 'email' es obligatorio para login privilegiado.");
        }

        // Comprobaciones de bloqueo por IP / usuario
        if (attemptService.isIpBlocked(ip)) {
            long retryAfter = attemptService.getIpRetryAfterSeconds(ip);
            logger.warn("[LOGIN-PRIVILEGED] IP bloqueada: {}. Retry-After: {}", ip, retryAfter);
            return ResponseEntity.status(429)
                .header(RETRY_AFTER_HEADER, String.valueOf(retryAfter))
                .body(DEMASIADOS_INTENTOS_LOGIN_IP + retryAfter + " " + SEGUNDOS_LITERAL + ".");
        }
        if (attemptService.isBlocked(username, ip)) {
            long retryAfter = attemptService.getUserRetryAfterSeconds(username, ip);
            logger.warn("[LOGIN-PRIVILEGED] Usuario bloqueado: {} desde IP: {}. Retry-After: {}", username, ip, retryAfter);
            return ResponseEntity.status(429)
                .header(RETRY_AFTER_HEADER, String.valueOf(retryAfter))
                .body(DEMASIADOS_INTENTOS_USUARIO + retryAfter + " " + SEGUNDOS_LITERAL + ".");
        }
        ...
        try {
            // Limpiar sesiones existentes del usuario antes de autenticar
            Optional<Usuario> existingUser = this.userRetrievalService.findAnyUserByEmail(username);
            existingUser.ifPresent(usuario -> {
                logger.info("[LOGIN-PRIVILEGED] Limpiando sesiones existentes para usuario privilegiado: {}", username);
                sesionService.eliminarTodasSesionesUsuario(usuario.getIdUsuario());
                sesionService.limpiarSesionesExpiradasEInvalidas();
                sesionService.limpiarTodasLasSesiones(usuario.getIdUsuario());
            });

            logger.info("[LOGIN-PRIVILEGED] Llamando a authenticatePrivilegedUser para usuario: {}", username);
            Usuario user = this.authenticationService.authenticatePrivilegedUser(credentials);
            logger.info("[LOGIN-PRIVILEGED] Autenticación exitosa para usuario privilegiado: {} con rol {}", username, user.getRol());
            
            // Reset de intentos fallidos
            attemptService.resetAttempts(username, ip);

            // Generar JWT (incluyendo tipoContenido en el caso de CreadorContenido)
            String jwt;
            ...
            logger.info("[LOGIN-PRIVILEGED] Token generado para usuario privilegiado: {}", username);
            
            // Rotar sesión por fixation (expira anteriores y crea nueva)
            sesionService.rotarSesionPorFixation(String.valueOf(user.getIdUsuario()), ip, jwt);
            logger.info("[LOGIN-PRIVILEGED] Sesión rotada para usuario privilegiado: {}", username);
            
            // Registrar sesión con absolute timeout
            sessionTimeoutService.registerSession(String.valueOf(user.getIdUsuario()), user.getRol());
            logger.info("[LOGIN-PRIVILEGED] Absolute timeout registrado para usuario privilegiado {} con rol {}", username, user.getRol());
            ...
            if (e.getStatusCode() == HttpStatus.FORBIDDEN) {
                logger.warn("[LOGIN-PRIVILEGED] Usuario privilegiado {} bloqueado: {}", username, e.getReason());
                throw e;
            }
            attemptService.recordFailedAttempt(username, ip);
            logger.error("[LOGIN-PRIVILEGED] Error de autenticación (ResponseStatusException) para usuario privilegiado: {} desde IP: {}", username, ip, e);
            throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, 
                ERROR_AUTENTICACION_PREFIX + username + FROM_IP_LITERAL + ip, e);
        }
        catch (Exception e) {
            attemptService.recordFailedAttempt(username, ip);
            logger.error("[LOGIN-PRIVILEGED] Error inesperado de autenticación para usuario privilegiado: {} desde IP: {}", username, ip, e);
            throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, 
                ERROR_AUTENTICACION_PREFIX + username + FROM_IP_LITERAL + ip, e);
        }
    }
