...
export class ConfigService {
  ...
  private getRuntimeConfig(): Partial<AppConfig> {
    try {
      // Opción 1: Desde sessionStorage (si se almacena configuración ahí)
      const storedConfig = sessionStorage.getItem('appConfig');
      if (storedConfig) {
        return JSON.parse(storedConfig);
      }

      // Opción 2: Desde variables globales (inyectadas por el servidor)
      const windowConfig = (globalThis as any).APP_CONFIG;
      if (windowConfig) {
        return windowConfig;
      }
