...
class NotificationServiceTest {
    ...
    @Test
    void userCanAccessContent_shouldReturnFalse_whenUserNotNormal() {
        usuarioNormal.setRol(Rol.ADMIN);
        boolean result = notificationService.userCanAccessContent(usuarioNormal, contenido); // vía reflexión o hazlo package-private

        assertFalse(result);
    }

    @Test
    void userCanAccessContent_shouldReturnFalse_whenContentNotVisible() {
        contenido.setVisibilidad(false);
        boolean result = notificationService.userCanAccessContent(usuarioNormal, contenido);

        assertFalse(result);
    }

    @Test
    void userCanAccessContent_shouldReturnFalse_whenVIPContentAndUserNotVIP() {
        contenido.setEsVIP(true);
        boolean result = notificationService.userCanAccessContent(usuarioNormal, contenido);
    ...
    @Test
    void userCanAccessContent_shouldReturnFalse_whenAgeRestrictionNotMet() {
        when(validationService.calculateAge(any())).thenReturn(16);
        contenido.setRestriccionEdad(RestriccionEdad.MAYOR_18);
        boolean result = notificationService.userCanAccessContent(usuarioNormal, contenido);
    ...
    @Test
    void matchesUserTastes_shouldReturnTrue_whenTagsMatch() {
        usuarioNormal.setGustosTags(List.of("musica", "rock"));
        contenido.setTags(List.of("pop", "rock"));

        boolean result = notificationService.matchesUserTastes(usuarioNormal, contenido); // vía reflexión

        assertTrue(result);
    }

    @Test
    void matchesUserTastes_shouldReturnFalse_whenNoMatch() {
        usuarioNormal.setGustosTags(List.of("musica"));
        contenido.setTags(List.of("pop", "jazz"));

        boolean result = notificationService.matchesUserTastes(usuarioNormal, contenido);
