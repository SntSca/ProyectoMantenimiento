...
class NotificationServiceTest {
    ...
    void testGetInbox() {
        ...
        assertThat(inbox.get(0).id()).isEqualTo("1");
    ...
    @Test
void testUserCannotAccessDueToWrongRole() {
    UsuarioNormal user = UsuarioNormal.builder()
            .idUsuario("u1")
            .rol(Rol.ADMINISTRADOR) // no NORMAL
            .build();
    ...
    boolean result = invokeUserCanAccess(user, contenido);
    assertThat(result).isFalse();
}

@Test
void testUserCannotAccessInvisibleContent() {
    UsuarioNormal user = UsuarioNormal.builder()
            .idUsuario("u1")
            .rol(Rol.NORMAL)
            .build();
...
@Test
void testUserCannotAccessVIPContentIfNotVIP() {
    UsuarioNormal user = spy(UsuarioNormal.builder()
            .idUsuario("u1")
            .rol(Rol.NORMAL)
            .build());

    doReturn(false).when(user).isFlagVIP();
...
@Test
void testUserCannotAccessDueToAgeRestriction() {
    UsuarioNormal user = UsuarioNormal.builder()
            .idUsuario("u1")
            .rol(Rol.NORMAL)
            .fechaNacimiento(java.sql.Date.valueOf("2010-01-01"))
            .build();
    ...
    when(validationService.calculateAge(any())).thenReturn(12);
...
@Test
void testUserCanAccessAllConditionsOK() {
    UsuarioNormal user = spy(UsuarioNormal.builder()
            .idUsuario("u1")
            .rol(Rol.NORMAL)
            .fechaNacimiento(java.sql.Date.valueOf("2000-01-01"))
            .build());
    doReturn(true).when(user).isFlagVIP();
...
/** Utilidad para invocar método privado */
private boolean invokeUserCanAccess(UsuarioNormal u, Contenido c) {
    try {
        var m = NotificationService.class.getDeclaredMethod("userCanAccessContent", UsuarioNormal.class, Contenido.class);
        m.setAccessible(true);
        return (boolean) m.invoke(notificationService, u, c);
    } catch (Exception e) {
        throw new RuntimeException(e);
    }
}
@Test
void testMatchesUserTastesNoGustos() {
    UsuarioNormal u = UsuarioNormal.builder().gustosTags(List.of()).build();
    Contenido c = Contenido.builder().tags(List.of("accion")).build();

    boolean r = invokeMatches(u, c);
    assertThat(r).isFalse();
}

@Test
void testMatchesUserTastesNoTags() {
    UsuarioNormal u = UsuarioNormal.builder().gustosTags(List.of("accion")).build();
    Contenido c = Contenido.builder().tags(List.of()).build();
...
@Test
void testMatchesUserTastesFalse() {
    UsuarioNormal u = UsuarioNormal.builder().gustosTags(List.of("drama")).build();
    Contenido c = Contenido.builder().tags(List.of("accion")).build();
...
@Test
void testMatchesUserTastesTrue() {
    UsuarioNormal u = UsuarioNormal.builder().gustosTags(List.of("accion")).build();
    Contenido c = Contenido.builder().tags(List.of("accion", "aventura")).build();

    boolean r = invokeMatches(u, c);
    assertThat(r).isTrue();
}

private boolean invokeMatches(UsuarioNormal u, Contenido c) {
    try {
        var m = NotificationService.class.getDeclaredMethod("matchesUserTastes", UsuarioNormal.class, Contenido.class);
        m.setAccessible(true);
        return (boolean) m.invoke(notificationService, u, c);
    } catch (Exception e) {
        throw new RuntimeException(e);
...
void testToDtoNullPayload() {
    ...
    NotificationDTO dto = invokeToDto(n);

    assertThat(dto.payload()).isEmpty();
    assertThat(dto.createdAt()).isNull();
}

@Test
void testToDtoWithPayloadAndDate() {
    ...
    assertThat(dto.payload()).containsEntry("raw", "raw-json");
    assertThat(dto.createdAt()).isEqualTo(now.toString());
}

private NotificationDTO invokeToDto(Notification n) {
    try {
        var m = NotificationService.class.getDeclaredMethod("toDto", Notification.class);
        m.setAccessible(true);
        return (NotificationDTO) m.invoke(notificationService, n);
    } catch (Exception e) {
        throw new RuntimeException(e);
    }
}
@Test
void testCreateAlertsForExpiringContent() {
    UsuarioNormal u = UsuarioNormal.builder()
            .idUsuario("u1")
            .rol(Rol.NORMAL)
            .gustosTags(List.of("accion"))
            .build();
    ...
    Contenido c = Contenido.builder()
            ...
            .esVIP(false)
            .tags(List.of("accion"))
            .fechaDisponibleHasta(java.sql.Date.valueOf(LocalDate.now().plusDays(3)))
            .build();

    when(usuarioNormalRepository.findAll()).thenReturn(List.of(u));
    when(validationService.calculateAge(any())).thenReturn(25);

    notificationService.createAlertsForExpiringContent(c);

    verify(notificationRepository, atLeastOnce()).save(any());
}
@Test
void testAlreadyHasRecommendedNotification() {
    when(notificationRepository.existsByUserIdAndSubtypeAndPayloadContaining("u1",
            NotificationSubtype.NEW_CONTENT_RECOMMENDED, "c1"))
            .thenReturn(true);

    boolean r = invokeExists("alreadyHasRecommendedNotification", "u1", "c1");
    assertThat(r).isTrue();
}

private boolean invokeExists(String method, String u, String c) {
    try {
        var m = NotificationService.class.getDeclaredMethod(method, String.class, String.class);
        m.setAccessible(true);
        return (boolean) m.invoke(notificationService, u, c);
    } catch (Exception e) {
        throw new RuntimeException(e);
